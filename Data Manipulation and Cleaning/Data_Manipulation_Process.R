...
sink("mcar_test_results.txt")
print(mcar_result)
sink() #to save the output as txt
...
imputed_handle_MD_df <- mice(handle_MD_df[, !names(handle_MD_df) %in% "New"], method="rf")  
  ...
  write.csv(imputed_handle_MD_df_final, "imputed_dataset_rf.csv", row.names=FALSE)
# Density plots 
ggplot(handle_MD_df, aes(x=hormone10_generated, fill="Original")) +
  ...
  labs(title="Density Plot of hormone10_generated: Original vs. Imputed (Random Forest)")+
...
write.csv(imputed_handle_MD_df_final1, "imputed_dataset_pmm.csv", row.names=FALSE)

# Density plots 
ggplot(handle_MD_df, aes(x=hormone10_generated, fill="Original")) +
geom_density(alpha=0.5) +
geom_density(data=imputed_handle_MD_df_final1, aes(x=hormone10_generated, fill="Imputed"), alpha=0.5) +
labs(title="Density Plot of hormone10_generated: Original vs. Imputed (PMM)")+
scale_x_continuous(limits = c(0, 2))
...
numeric_data <- imputed_handle_MD_df_final %>% select(where(is.numeric)) %>% na.omit()

# Calculate LOF scores
lof_scores <- lof(numeric_data, minPts = 5)

# Histogram of LOF scores
ggplot(data.frame(LOF = lof_scores), aes(x = LOF)) +
  geom_histogram(bins = 30, fill = "lightblue", color = "black") +
  labs(title = "Histogram of LOF Scores",
       x = "LOF Score",
       y = "Count") +
  theme_minimal()

# Bivariate scatterplot with outliers highlighted
ggplot(data.frame(numeric_data, LOF = lof_scores), 
       aes(x = lipids1, y = lipids2, color = LOF > 2)) +
  geom_point(alpha = 0.7) +
  scale_color_manual(values = c("FALSE" = "blue", "TRUE" = "red"),
                     labels = c("Normal", "Outlier")) +
  labs(title = "Bivariate Scatterplot with Outliers (LOF)",
       color = "Status") +
  theme_minimal()
