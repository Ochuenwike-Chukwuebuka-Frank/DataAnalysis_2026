...
#--------------------------------------------------
#Task 2: detailed analysis for the Bray-Curtis metric
#--------------------------------------------------

#2.1: NMDS with Bray-Curtis distance (repeat for clarity)

ord_bray <- metaMDS(data, distance = "bray", trymax = 50)

#2.2: Vectors of significant species using envfit (p ≤ 0.05)
...
# 2.4. Select 2-3 clusters (choose the number visually or by silhouette)
# Here, for example, we will take k = 2 (it can be changed to 3 after viewing the dendrogram)
k_clusters <- 2
clusters <- cutree(hc_bray, k = k_clusters)

# Let's turn it into a factor for convenience
clusters <- factor(clusters, labels = c("Cluster 1", "Cluster 2"))

# 2.5. Plotting the final NMDS graph with ellipses and vectors
# color scheme
...
#--------------------------------------------------------
# Task 3. Testing for differences between clusters (PERMANOVA)
#--------------------------------------------------------

# 3.1. Performing PERMANOVA with 999 permutations
set.seed(456)
...
# We derive R² and p-value
R2 <- adonis_result$R2[1]
p_val <- adonis_result$`Pr(>F)`[1]

# 3.3. Output to screen
cat("\n=======================================\n")
cat("PERMANOVA results (Bray-Curtis):\n")
cat("R² =", round(R2, 4), "\n")
cat("p-value =", p_val, "\n")
if (p_val < 0.05) {
  cat("Conclusion: the differences between clusters are statistically significant (p < 0.05).\n")
} else {
  cat("Conclusion: differences between clusters are not significant (p >= 0.05).\n")
}
cat("=======================================\n")

# Additionally: let's see which samples fell into which clusters
cat("\nCluster composition:\n")
